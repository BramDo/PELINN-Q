...
class NoisyExecutorPool:
    ...
    def get_executor(self, noise_cfg: Dict, observable: SparsePauliOp, shots: int | None):
        ...
        if key not in self._cache:
            ...
            if BackendEstimator is not None:
        ...
        def _executor(qc) -> float:
            ...
            if estimator is not None:
                ...
                # try typical result shape
                try:
                    return float(result.values[0])
                except Exception:
                    return float(result[0])
            # Fallback: run AerSimulator and compute expectation from counts/statevector
            job = backend.run(compiled, shots=eff_shots)
            res = job.result()
            try:
                counts = res.get_counts()
                from pelinn.data.qiskit_dataset import _expectation_from_counts
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    # Ensure we have a usable Estimator primitive available; older/newpackaged
    # qiskit installations may not expose `BackendEstimator`/`Estimator`.
    if BackendEstimator is None:
        logging.warning(
            "qiskit.primitives BackendEstimator/Estimator not available in this environment.\n"
            "Falling back to Aer-based executor/counts/statevector fallbacks; some baselines may be disabled."
        )
